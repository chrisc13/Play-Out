...
class LoginViewController: UIViewController, UITextFieldDelegate,  ActivityIndicatorPresenter {
                    ...
                    case .success(let granted) :
                            if granted{
                                ...
                                self.getUserData( completion: { result in
                                    ...
                                    case .failure( _):  self.hideActivityIndicator();  Alert.showBasicAlert(on: self, with: "Error logging in", message: "", actionName: "Okay")
    ...
    func getProfilePicture(uid:String, completion: @escaping(UpResult) ->() ){
        ...
        imgRef.getData(maxSize: 1 * 1024 * 1024) { data, error in
            if let error = error {
               ...
               // guard let pic = img.resizeImageUsingVImage(size: CGSize(width: 100, height: 100)) else {return}
                //UserDefaults.standard.set(pic, forKey: "currUserImage")
